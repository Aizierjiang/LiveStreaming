...
namespace Unity.Ig.VideoStreaming.Server
{
    public interface IH264Encoder
    {
        byte[] GetRawSPS();
        byte[] GetRawPPS();
        void CompressFrame(NativeArray<byte> data, ref ArraySegment<byte> compressedData, out bool isKeyFrame);
        bool ExpectsYUV();
    }
    ...
    public class RtspServer : IDisposable
    {
        ...
        private IH264Encoder h264_encoder = null;
        ...
        private void AddSTAPANalu(in NativeList<byte> nalu, int naluStartByteIdx, int naluEndByteIdx, ref NativeList<byte> rtp_packet, bool includeSize = true)
        {
            ...
            for (var i = naluStartByteIdx; i < naluEndByteIdx; ++i)
                rtp_packet.Add(nalu[i]);
        }

        public void SendNALUs(ulong timeStampNs, in NativeList<byte> spsNalu, in NativeList<byte> ppsNalu, in NativeList<byte> imageNalu)
        {
            ...
            for (int x = 0; x < nal_array.Length; x++)
            {
                ...
                if (!raw_nal.IsCreated || raw_nal.Length == 0)
                    continue;
                ...
                if (fragmenting == false)
                {
                    ...
                    byte[] rtp_packet = new byte[12 + stapHeaderSize + raw_nal.Length]; // 12 is header size when there are no CSRCs or extensions
                    ...
                    if (enableSTAP && !last_nal)
                    {
                        ...
                        NativeArray<byte>.Copy(raw_nal, 0, rtp_packet, 12, raw_nal.Length);
                        rtp_packets.Add(rtp_packet);
                    }
                    else if (enableSTAP)
                    {
                        var rtpPacket = new NativeList<byte>(rtp_packet.Length, Allocator.Temp);
                        for (int i = 0; i < 12; ++i)
                        ...
                        int naluCount = 0;
                        for (var i = 0; i < raw_nal.Length;)
                        {
                            nalEndByteIdx = i;
                            if (raw_nal[i++] != 0)
                                continue;
                            if (i >= raw_nal.Length || raw_nal[i++] != 0)
                                continue;
                            if (i >= raw_nal.Length || raw_nal[i++] != 0)
                                continue;
                            if (i >= raw_nal.Length || raw_nal[i++] != 1)
                                continue;
                            ...
                            {
                                AddSTAPANalu(raw_nal, nalStartByteIdx, nalEndByteIdx, ref rtpPacket);
                                ++naluCount;
                            }
                        ...
                        if (nalStartByteIdx < (raw_nal.Length - 1))
                        {
                            //Debug.Log("Handling ending NALU");
                            AddSTAPANalu(raw_nal, nalStartByteIdx, raw_nal.Length, ref rtpPacket);
                            ++naluCount;
                        }
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < raw_nal.Length;)
                        {
                            ...
                            var rtpPacket = new NativeList<byte>(Allocator.Temp);
                            for (int j = 0; j < 12; ++j)
                            ...
                            if (nalStartByteIdx < nalEndByteIdx)
                            {
                                AddSTAPANalu(raw_nal, nalStartByteIdx, nalEndByteIdx, ref rtpPacket, false);
                ...
                else
                {
                    ...
                    while (data_remaining > 0)
                    {
                        ...
                        NativeArray<byte>.Copy(raw_nal, nal_pointer, rtp_packet, 14, payload_size);
        ...
        {
            Profiler.BeginSample("Connection list cleanup");
        ...
        public void video_source_ReceivedYUVFrame(uint timestamp_ms, in NativeArray<byte> image_data)
        {
            if (!RefreshConnectionList())
                return;

            Profiler.BeginSample("H.264 encoding");
            Boolean isKeyframe; // SimpleH264encoder and TinyH24encoder only emit keyframes

            // Compress the video (YUV to H264)
            h264_encoder.CompressFrame(image_data, ref raw_video_nal, out isKeyframe);
            Profiler.EndSample();
            ...
            NativeList<byte> rawSps = default;
            NativeList<byte> rawPps = default;

            if (add_sps_pps_to_keyframe && isKeyframe)
            {
                // FIXME: Mega convoluted multi-copy to go from ArraySegment to NativeList...
                // NativeList.CopyFrom just doesn't work...
                var spsArray = h264_encoder.GetRawSPS();
                var x = new NativeArray<byte>(spsArray, Allocator.Temp);
                rawSps = new NativeList<byte>(spsArray.Length, Allocator.Temp);
                rawSps.AddRange(x);

                var ppsArray = h264_encoder.GetRawPPS();
                x = new NativeArray<byte>(ppsArray, Allocator.Temp);
                rawPps = new NativeList<byte>(ppsArray.Length, Allocator.Temp);
                rawPps.AddRange(x);
            }

            var a = new NativeArray<byte>(raw_video_nal.Count, Allocator.Temp, NativeArrayOptions.UninitializedMemory);
            NativeArray<byte>.Copy(raw_video_nal.Array, raw_video_nal.Offset, a, 0, raw_video_nal.Count);
            var imageNalu = new NativeList<byte>(raw_video_nal.Count, Allocator.Temp);
            imageNalu.AddRange(a);
            SendNALUs(timestamp_ms * 90, in rawSps, in rawPps, in imageNalu); // 90kHz clock
        }
